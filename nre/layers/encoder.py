...
class Encoder(object):
    ...
    def encode(self, is_train, mask, model_inputs):
        """Interface for encoding"""
        return self.selected_encoder(is_train, mask, model_inputs)

    def cnn1_layer(self, is_train, mask, model_inputs, name=None):
    ...
    def cnn2_layer(self, is_train, mask, model_inputs,  name=None):
        with tf.name_scope('CNN'):
            model_inputs = tf.expand_dims(model_inputs, axis=1)
            if FLAGS.use_type:
    ...
    def pcnn_layer(self, is_train, mask, model_inputs, name=None):
